...
import { Funnel, FunnelLevel, AgeDistribution } from '@/types';
import { cn, getRelativeTime, generateId, formatNumber } from '@/lib/utils';

// ── Population group helpers ─────────────────────────────────────────────────
...
function getStartingPop(countryCode: string, group: PopGroupKey): { value: number | undefined; name: string } {
  ...
  const value = !c ? undefined : group === 'total' ? c.population : c.ageDistribution[group as keyof AgeDistribution];
  return { value, name: POP_GROUP_LABELS[group] };
}

// ── FunnelPanel ──────────────────────────────────────────────────────────────
// Defined OUTSIDE the page component so the reference is stable across renders.
...
interface FunnelPanelProps {
  ...
  onSave: () => void;
  onCloseCompare: () => void;
  onLevelClick: (funnelId: string, level: FunnelLevel) => void;
}
...
function FunnelPanel({
  ...
  const cd = getCountry(funnel.country);
  return (
    <>
      {/* Header */}
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {isPrimary ? (
            <>
              ...
              {adminMode && (
                <>
                  <Button size="sm" variant="secondary" leftIcon={<Plus className="w-3.5 h-3.5" />} onClick={onAddLevel}>Add Level</Button>
                  <Button size="sm" variant="primary" leftIcon={<Save className="w-3.5 h-3.5" />} onClick={onSave}>Save</Button>
                </>
              )}
            </>
          ) : (
      ...
      {/* Admin banner */}
      {isPrimary && adminMode && (
        <div className="px-5 py-1.5 bg-accent-muted border-b border-accent/20">
      ...
      <div className="px-4 py-3">
        <FunnelVisualization
          funnel={funnel}
          onLevelClick={(level) => onLevelClick(funnel.id, level)}
...
export default function PatientFunnelsPage() {
  ...
  const handleChatSend = () => {
    ...
    const idx = Math.min(chatMessages.filter((m) => m.role === 'ai').length, replies.length - 1);
    setTimeout(() => setChatMessages((prev) => [...prev, { role: 'ai', content: replies[idx] }]), 800);
  };

  const handleCreateFunnel = () => {
    if (!newFunnelName.trim()) return;
    const { value: startPop, name: levelName } = getStartingPop(newFunnelCountry, newFunnelGroup);
    const country = getCountry(newFunnelCountry);
      ...
      levels: [
        {
          id: `level-${generateId()}`,
          name: levelName,
          description: country ? `${levelName} — ${country.name} (Source: ${country.dataSource})` : levelName,
          percentage: 100,
          value: startPop,
          linkedArticles: [],
        },
  ...
  const filteredFunnels = useMemo(() => funnels.filter((f) => {
  ...
  // ── Population preview (new funnel modal) ─────────────────────────────────
  const selectedCountry = getCountry(newFunnelCountry);
    ...
    <AuthGuard>
      <AppShell>
        <div className="flex flex-col h-full overflow-hidden">
          ...
          {/* ── Funnel Workspace ──────────────────────────────────────────── */}
          <div className="flex-1 border-b border-border overflow-auto min-h-0">
            {!activeFunnel ? (
              ...
              <div className="grid grid-cols-2 divide-x divide-border">
                <div className="overflow-auto">
                  <FunnelPanel
                    ...
                    adminMode={adminMode}
                    canEdit={canEdit}
                    onToggleAdmin={() => setAdminMode((p) => !p)}
                    onAddLevel={() => addLevel(activeFunnel.id, { name: 'New Level', description: '', percentage: 0, linkedArticles: [] })}
                    onSave={() => updateFunnel(activeFunnel.id, {})}
                    onCloseCompare={() => setCompareFunnelId(null)}
            ...
            <div className="px-5 py-2.5 border-b border-border flex items-center gap-3 flex-wrap sticky top-0 bg-card z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  ({filteredFunnels.length}{filteredFunnels.length !== funnels.length && `/${funnels.length}`})
                </span>
              </div>
                      ...
                      <tr key={funnel.id} className={cn(isActive && 'bg-accent-muted', isCompare && 'bg-muted')}>
                        ...
                        <td>
                          <div className="flex items-center gap-2">
                            ...
                            {/* Compare — only when a different primary funnel is active */}
                            {activeFunnel && funnel.id !== activeFunnelId && (
                              <button
                                ...
                                className={cn(
                                  ...
                                  isCompare
                                    ? 'text-exclude hover:text-exclude/80'
            ...
            <div className="space-y-4">
              ...
              <div className="bg-muted rounded-md p-3 space-y-2 max-h-36 overflow-y-auto">
                {chatMessages.map((msg, idx) => (
                  <div
                    key={idx}
                    className={cn('text-xs rounded-md px-3 py-2 max-w-[85%]', msg.role === 'ai' ? 'bg-card border border-border text-foreground' : 'bg-accent text-white ml-auto')}
                  >
                    {msg.content}
                  </div>
                ))}
              ...
              <div className="border-t border-border pt-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  />
                  <Select
                    label="Starting Population Group"
                    value={newFunnelGroup}
                    onValueChange={(v) => setNewFunnelGroup(v as PopGroupKey)}
                    options={POP_GROUP_OPTIONS}
                  />
                </div>
                ...
                {selectedCountry && (
                  <div className="rounded-lg border border-border bg-muted p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                      {POP_GROUPS.map((g) => {
                        const pop = g.value === 'total'
                          ? selectedCountry.population
                          : selectedCountry.ageDistribution[g.value as keyof AgeDistribution];
                    ...
                    {previewPop !== undefined && (
                      <p className="text-[11px] text-muted-foreground">
                        First level will be initialised with{' '}
                        <span className="font-semibold text-foreground">{formatNumber(previewPop)}</span>
                        {' '}({POP_GROUP_LABELS[newFunnelGroup]}).
                      </p>
                    )}
                  </div>
                )}
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button variant="primary" onClick={handleCreateFunnel} disabled={!newFunnelName.trim()}>
                  Create Funnel
                </Button>
              </div>
            </div>
